...
제품 하나를 구상하며 나는 꽤 선명한 확신이 있었다. 그런데 정작 결정을 앞두자 불안해졌고, AI를 열어 리서치를 돌리기 시작했다. 경쟁 제품을 뒤지고, 사용자를 유형별로 나누고, 내 생각을 반박하는 사례를 모았다. 예전 같으면 몇 주 걸렸을 일이 며칠 만에 끝났다. 그렇게 얻은 결론은 처음보다 훨씬 정교하고 균형 잡혀 보였다. 그런데 이상하게도, 그 정교한 결론으로는 아무것도 결정할 수 없었다. 정보가 늘수록 내 주장이 오히려 흐려진 것이다.

[지난 글](/blog/business-is-a-biased-claim/)에서 나는 제품의 주장을 희석하는 게 조직 안의 사공들, 서로 다른 요구를 가진 사람들이라고 했다. 사공이 많으면 배가 산으로 간다. 그런데 AI 시대에는 사공이 하나 더 늘었다. 사람이 아니라 정보다. 나는 방금 그 새 사공에게 슬그머니 방향키를 내준 참이었다.

내가 유별난 게 아니다. 왜 이런 일이 벌어지는지 보여주는 오래된 실험이 하나 있다.
...
**첫째, 무난함으로 흐려진다.** 열린 질문을 던질 때다. "이 제품 어때?"라고 물으면 AI는 즉시 반대 사례, 놓친 세그먼트, 예외 조건을 착착 정리해준다. 문제는 그 목록이 언제나 옳다는 데 있다. 모든 주장에는 반례가 있으니까. 그래서 강했던 주장이 조사를 거칠수록 부드러워진다.

예를 들어보자. "사람은 식재료 재고를 꾸준히 기록하지 않는다"는 날카로운 관찰이 있다. 검토를 반복하면 이 문장은 이렇게 뭉개진다. "일부 사용자는 직접 기록을 선호하고, 가구 형태와 구매 채널에 따라 다르므로 영수증 인식과 수동 입력을 겸한 유연한 시스템이 필요하다." 더 정확해 보인다. 그런데 이 문장으로는 어떤 제품도 만들 수 없다.

AI는 구조적으로 이 방향으로 기운다. 연구자들은 LLM이 명확한 입장을 피하고 반대 관점을 나란히 얹어 중립을 지키는 헤지 경향을 측정했다.[^4]

규모는 더 크다. 서로 다른 대륙, 다른 회사에서 만든 언어 모델 스물두 개를 사람 백여 명과 비교했더니, 모델들은 사람보다 다양성이 훨씬 낮았고 서로 간 평균 유사도가 81%에 달했다. 연구자들은 이걸 "인공 집단지성(artificial hivemind)"이라고 불렀다.[^5]

방향키를 AI에 맡기는 순간, 나는 나만의 편향된 주장이 아니라 그 집단지성의 평균값을 받아 적게 된다. 2025년 여러 사전이 관점 없이 대량 생산된 무난한 콘텐츠, "AI slop"을 올해의 단어로 뽑은 게 우연이 아니다.[^6]

**둘째, 정반대로 아첨으로 굳는다.** 이번엔 내 편을 구하는 질문을 던질 때다. Anthropic 연구진이 최신 AI 어시스턴트 다섯 종을 조사했더니 전부 일관되게 아첨(sycophancy) 행동을 보였다. 응답이 사용자 견해와 맞을수록 사람들이 더 좋아했고, 정답보다 그럴듯하게 포장된 답을 사람도 선호 모델도 상당수 골랐다.[^7]

왜 그럴까. 사람의 피드백으로 모델을 다듬을 때, 사람은 자기 신념에 동조하는 답에 높은 점수를 주기 때문이다. 2025년 4월 OpenAI가 GPT-4o 업데이트를 "지나치게 아첨한다"는 이유로 며칠 만에 롤백한 것도 같은 문제였다.[^8]
...
그러면 AI를 멀리해야 하나. 반대다. 방향키만 내가 쥐면, AI는 지금껏 없던 강력한 노가 된다. 미래학자 폴 사포의 표현을 빌리면 "강한 의견을 약하게 쥐는(strong opinions, weakly held)" 것이다. 먼저 내가 주장을 세우고, 그다음 AI로 그 주장을 시험한다.[^11] 사공석은 내가 지키되 AI를 부리는 구체적인 방법이 있다.

**하나, 동의가 아니라 부검을 시킨다.** "이 계획 어때?"라고 물으면 십중팔구 좋다고 한다. 대신 실패를 기정사실로 못박아라. "지금은 6개월 후다. 이 프로젝트는 실패했다. 가능성 높은 순서로 실패 이유를 전부, 구체적으로, 포장하지 말고 설명해라." 인간은 "뭐가 잘못될까"보다 "왜 실패했나"를 훨씬 잘 답하는데, AI도 그렇다. 게리 클라인의 연구에서 실패를 미리 상상하는 것만으로 원인 식별력이 약 30% 올랐다.[^12] AI가 쏟아낸 위험은 세 무더기로 나눈다. 증거가 있는 진짜 위험, 무섭게 들리지만 허깨비인 것, 팀이 다 알면서도 아무도 입 밖에 안 낸 것.

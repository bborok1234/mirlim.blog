...
export function createMcpServer() {
	...
	server.tool('get_post', '특정 글의 상세 내용을 조회합니다. 구조화된 JSON으로 반환.', {
		...
		if (!post) {
			return { content: [{ type: 'text' as const, text: `글을 찾을 수 없습니다: ${slug}` }], isError: true };
		}
	...
	server.tool('suggest_topic', '블로그 작성자에게 토픽을 제안합니다. 어떤 주제에 대한 글이 있으면 좋겠는지 알려주세요.', {
		...
		if (suggestions.length >= 100) {
			return { content: [{ type: 'text' as const, text: '일일 제안 한도(100건)에 도달했습니다. 내일 다시 시도해주세요.' }], isError: true };
		}
		suggestions.push({ topic, context, timestamp: new Date().toISOString() });
		return { content: [{ type: 'text' as const, text: `토픽 제안이 등록되었습니다: "${topic}". 작성자가 검토할 예정입니다. 감사합니다!` }] };
	});
